...
export default function NsmSalesPerformanceDrillPanel({ token, caller, onClose, initialFilter = 'assigned' }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch('/api/admin/nsm/leads', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
      fetch(`/api/admin/nsm/calls?caller_id=${caller.caller_id}&limit=200`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
