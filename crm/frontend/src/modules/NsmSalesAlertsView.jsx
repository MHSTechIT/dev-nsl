...
export default function NsmSalesAlertsView({ token }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [alertsRes, usersRes] = await Promise.all([
        fetch('/api/admin/nsm/telegram-alerts', { headers: { Authorization: `Bearer ${token}` } }),
  ...
  async function removeRow(idx) {
    ...
    try {
      const res = await fetch(`/api/admin/nsm/telegram-alerts/${row.id}`, {
        method: 'DELETE',
    ...
    try {
      for (const row of rows) {
        if (row._draft) {
          ...
          const res = await fetch('/api/admin/nsm/telegram-alerts', {
            method: 'POST',
  ...
  async function sendTest(row) {
    ...
    setTestingId(row.id);
    try {
      const res = await fetch(`/api/admin/nsm/telegram-alerts/${row.id}/test`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
