...
/* ──────────────────────────────────────────────────────────────────────
   ...
   Powered by:
     GET /api/admin/nsm/completed-calls   (auth: ADMIN_PASSWORD bearer)
...
const COLUMN_FILTERS = {
  ...
  webinar_name:                    { type: 'select', label: 'Batch' },
};
...
export default function NsmSalesCompletedCallsView({ token }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/admin/nsm/completed-calls', {
  ...
  // Fetch callers + batches once for the dropdown options.
  useEffect(() => {
    ...
    fetch('/api/admin/nsm/batches', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => setWebinars((d.batches || []).map(b => ({ id: b.id, name: b.batch_name }))))
      .catch(() => {});
  }, [token]);
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Outfit, sans-serif' }}>
              <thead ref={headerRef}>
                <tr style={{ background: 'rgba(237,234,248,0.50)', textAlign: 'left' }}>
                  ...
                  <FilterableTh label="Batch"                colKey="webinar_name"                    {...filterProps()} />
