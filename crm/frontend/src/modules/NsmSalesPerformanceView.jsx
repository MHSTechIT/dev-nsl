...
import NsmSalesPerformanceDrillPanel from './NsmSalesPerformanceDrillPanel';
import NsmReassignDistributionModal   from '../admin/NsmReassignDistributionModal';
import NsmCallerActivityDrawer        from '../admin/NsmCallerActivityDrawer';
import NsmCallerPageDrawer            from '../admin/NsmCallerPageDrawer';
...
/* ══════════════════ Main component ══════════════════ */
export default function NsmSalesPerformanceView({ token, actionsSlotEl }) {
  ...
  async function confirmExport() {
    ...
    try {
      ...
      const r = await fetch(`/api/admin/nsm/sales-performance/leads-export?${params}`, {
  ...
  useEffect(() => {
    fetch('/api/admin/nsm/users', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => {
  ...
  /* Batch list for the dropdown — NSM uses batches in place of webinars.
     Each batch is mapped to the same { id, name } shape the dropdown
     expects (name ← batch_name); is_active:true so the inactive label
     logic never flags a batch. The chosen batch id is still passed as
     the `webinar_id` query param unchanged. */
  useEffect(() => {
    fetch('/api/admin/nsm/batches', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => setWebinars((d.batches || []).map(b => ({ id: b.id, name: b.batch_name, is_active: true }))))
      .catch(() => {});
  }, [token]);
  ...
  const fetchData = useCallback(async () => {
    ...
    if (webinarId) params.set('webinar_id', webinarId);
    try {
      const res = await fetch(`/api/admin/nsm/sales-performance?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  async function confirmMoveScope({ scope, date }) {
    ...
    closeMovePicker();
    try {
      const url = `/api/admin/nsm/caller-workload?date=${encodeURIComponent(date)}`;
  ...
  async function togglePause(row) {
    ...
    try {
      const res = await fetch(`/api/admin/nsm/users/${id}`, {
        method:  'PATCH',
  ...
  return (
    <div style={{ fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div className="sp-filter-bar" style={{
        ...
        <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'rgba(91,33,182,0.65)' }}>Batch</span>
