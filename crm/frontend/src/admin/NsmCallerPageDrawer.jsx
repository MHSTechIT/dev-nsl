...
export default function NsmCallerPageDrawer({ token, callerId, callerName, onClose, onAfterReopen }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    fetch(`/api/admin/nsm/caller-leads/${callerId}`, {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/nsm/batches', { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(d => { if (!cancelled) setWebinars((d.batches || []).map(b => ({ id: b.id, name: b.batch_name }))); })
  ...
  async function handleReopen() {
    ...
    setBusy(true);
    try {
      const res = await fetch('/api/admin/nsm/leads/reopen', {
        method:  'POST',
